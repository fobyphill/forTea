...
def dcsp(contract_id, code, headers, user_id, dict_object, dict_edit_object, timestamp, parent_transact, *list_contracts, **params):
    ...
    # 3. Отработаем триггеры
        ...
        # # Выполним сохранение техпроцессов
        # #  Сохраняем только при редактировании. При создании сохраняем в другом месте
        # if 'tps' in params and params['tps'] and dict_object != dict_edit_object:
        #     result = save_edited_tps(params['tps'], presaved_object, dict_edit_object, timestamp, user_id, parent_transact)
        #     if result != 'ok':
        #         raise Exception(result)
