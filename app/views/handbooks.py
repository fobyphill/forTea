...
from app.functions import convert_funs, common_funs, reg_funs, database_funs, session_funs, database_procedures, \
    ...
    convert_procedures, view_procedures, class_funs
...
def manage_object(request):
    ...
    if 'b_save' in request.POST:
        ...
        if is_saved and not request.POST['i_code']:
            ...
            request.POST['object_code'] = str(code)
    ...
    # кнопка "Удалить файл"
        ...
        if not interface_funs.delete_file(request, current_class, headers):
    ...
    # Кнопка "Удалить"
        ...
        if 'i_code' in request.POST and request.POST['i_code']:
            ...
            res_check_child = api_procedures.fc4c(current_class, code, 'table')
            ...
            if delete_valid:
                ...
                for task in Tasks.objects.filter(data__class_id=class_id, data__code=code, data__location='t'):
                    task_funs.delete_simple_task(task, timestamp, request.user.id, parent_transact=transaction_id)
    ...
    # контрольные пересчеты
    totals = convert_procedures.gtfol(objects, visible_headers)  # Итоги
    session_funs.check_quant_tasks(request)  # Задачи
    session_funs.crqd(request)  # черновик
    # Периоды таймлайна
        ...
        if not delete_folder:
            ...
            if is_valid:
                ...
                request.session['draft_menu'] = session_funs.update_class_menu(request,
                                                                               ...
                                                                               is_draft=True)
                session_funs.update_draft_tree(request)  # обновим меню черновиков
                class_funs.dndi_frohi_toarhiv(id, 't', deleted_type)  # Перемещение данных из истории в архив
            else:
                message_class = 'text-red'
...
# Устаревшее. Удалить после 01.06.2025
