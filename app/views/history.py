...
from app.functions import common_funs, session_funs, view_procedures
from app.models import RegName, RegistratorLog, TechProcess, Arhiv
        ...
        if json_href and 'type' in json_href and 'class_id' in json_href:
            ...
            if 'code' in json_href:
                if json_href['type'] == 'dict':
                    ...
                    href = 'contract'
                elif json_href['type'] in ['custom', 'prop', 'cotc']:
                    href = ''
                else:
                ...
                if json_href['type'] == 'tp':
                    parent_class_hist = RegistratorLog.objects.filter(reg_name_id__in=(1,3), json__type='tp',
                                                                      json_class=json_href['class_id']).order_by('-id')[:1]
                    if parent_class_hist:
                        obj_class_id = str(parent_class_hist[0].json['parent'])
                    else:
                        obj_class_id = '0'
                else:
...
def arhiv(request):
    ...
    one_month_ago = one_month_ago.strftime('%Y-%m-%dT%H:%M:%S')
    now = now.strftime('%Y-%m-%dT%H:%M:%S')
    date_in = request.GET['i_date_in'] if 'i_date_in' in request.GET and request.GET['i_date_in'] else one_month_ago
    date_out = request.GET['i_date_out'] if 'i_date_out' in request.GET and request.GET['i_date_out'] else now

    arhiv = Arhiv.objects.using('arhiv').filter(date_update__gte=date_in, date_update__lte=date_out)\
        .annotate(user_data=Value('', output_field=CharField()))\
        .annotate(reg_data=Value('', output_field=CharField())).annotate(data=Value('', output_field=CharField()))

    is_valid = True
    # Регистратор
    if 'i_filter_reg' in request.GET and request.GET['i_filter_reg']:
        try:
            reg_id = int(request.GET['i_filter_reg'])
        except ValueError:
            message += 'Некорректно указан ID регистратора. Укажите целое число\n'
            is_valid = False
            reg_id = 0
        arhiv = arhiv.filter(reg_id=reg_id)
    # айди класса
    if 'i_filter_class' in request.GET and request.GET['i_filter_class']:
        try:
            class_id = int(request.GET['i_filter_class'])
        except ValueError:
            message += 'Некорректно указан ID класса. Укажите целое число\n'
            is_valid = False
            class_id = 0
        arhiv = arhiv.filter(class_id=class_id)
    # локация
    if 's_location' in request.GET and request.GET['s_location'] != 'all':
        arhiv = arhiv.filter(location=request.GET['s_location'])
    # тип
    if 's_type' in request.GET and request.GET['s_type'] != 'all':
        arhiv = arhiv.filter(type=request.GET['s_type'])
    # код
    if 'i_filter_object' in request.GET and request.GET['i_filter_object']:
        try:
            code = int(request.GET['i_filter_object'])
            ...
            is_valid = False
            code = 0
        arhiv = arhiv.filter(code=code)
    # заголовок
    if 'i_header' in request.GET and request.GET['i_header']:
        try:
            header = int(request.GET['i_header'])
        except ValueError:
            message += 'Некорректно указан заголовок. Укажите целое число\n'
            is_valid = False
            header = 0
        arhiv = arhiv.filter(header_id=header)
    # транзакция
    if 'i_transact' in request.GET and request.GET['i_transact']:
        arhiv = arhiv.filter(transact_id=request.GET['i_transact'])
    if 'i_parent_transact' in request.GET and request.GET['i_parent_transact']:
        arhiv = arhiv.filter(transact_id=request.GET['i_parent_transact'])
    # юзер
    if 'i_user' in request.GET and request.GET['i_user']:
        try:
            user_id = int(request.GET['i_user'])
        except ValueError:
            message += 'Некорректно указан ID пользователя. Укажите целое число\n'
            is_valid = False
            user_id = 0
        arhiv = arhiv.filter(user_id=user_id)
    if not is_valid:
         message_class = 'text-red'
    paginator = common_funs.paginator_standart(request, arhiv)
    # постобработка
    users = get_user_model().objects.filter(id__in=[ip.user_id for ip in paginator['items_pages']]) \
        .values('id', 'first_name', 'last_name')
    reg_names = request.session['temp_data']['data']['reg_names']
    ...
    for ip in paginator['items_pages']:
        ...
        if current_user['id'] != ip.user_id:
            current_user = next(u for u in users if u['id'] == ip.user_id)
        ip.user_data = current_user['first_name'] + ' ' + current_user['last_name']
        ip.reg_data = next(rn['name'] for rn in reg_names if rn['id'] == ip.reg_id)
        data = 'in={'
        if ip.value_inc:
            data += f'{ip.value_inc}'
        data += '}\nout={'
        if ip.value_outc:
            data += f'{ip.value_outc}'
        data += '}'
        ip.data = data
    session_funs.check_quant_tasks(request)
    ctx = {
    ...
    return render(request, 'history/arhiv.html', ctx)